...
├── src/java/
│   ├── constant/
│   │   └── Iconstant.java               # Google OAuth2 credentials (hardcoded)
│   │
│   ├── controller/                      # Primary servlet package (~40 servlets)
│   │   ├── HomePageServlet.java
│   │   ├── doctor/
│   │   │   ├── QueueManager.java        # Today's patient queue for the doctor
│   │   │   ├── DoctorAppointmentDetail.java
│   │   │   ├── DoctorTimeSlotListServlet.java
│   │   │   ├── UpdateDoctorTimeSlotServlet.java
│   │   │   ├── AddUnavailabilityServlet.java
│   │   │   ├── RemoveUnavailabilityServlet.java
│   │   │   ├── MedicalRecordsServlet.java
│   │   │   ├── MedicalRecordsJsonServlet.java  # AJAX endpoint
│   │   │   └── TransferPatient.java     # Doctor handoff
│   │   ├── manager/
│   │   │   ├── ManagerAnalyticServlet.java
│   │   │   ├── ManagerAppointmentAnalyticServlet.java
│   │   │   ├── AccountManager.java
│   │   │   ├── StaffManager.java
│   │   │   ├── ExaminationPackageManager.java
│   │   │   ├── ManagerTimeSlotListServlet.java
│   │   │   └── *DoctorAssignmentServlet.java (Insert/Update/Delete)
│   │   ├── patient/
│   │   │   ├── PatientBookAppointmentServlet.java
│   │   │   ├── PatientExaminationPackageSelectServlet.java
│   │   │   ├── DoctorBySlotServlet.java # AJAX: available doctors for a slot
│   │   │   ├── PatientAppointmentsListServlet.java
│   │   │   ├── UpdateAppointmentServlet.java
│   │   │   ├── DeleteAppointmentServlet.java
│   │   │   ├── PatientQueueViewer.java
│   │   │   ├── InvoiceDetailsServlet.java
│   │   │   ├── PaymentBookingServlet.java   # Builds VNPay redirect URL
│   │   │   ├── VnpayReturnBooking.java      # VNPay HMAC callback handler
│   │   │   └── Config.java                  # VNPay config + HMAC-SHA512 util
│   │   ├── staff/
│   │   │   ├── AppointmentManager.java
│   │   │   ├── PatientCheckin.java
│   │   │   ├── PatientListServlet.java
│   │   │   ├── ReceptionAppointmentsListServlet.java
│   │   │   └── PatientMedicalRecordsServlet.java
│   │   └── user/
│   │       ├── LoginServlet.java        # Email/password + Google OAuth2
│   │       ├── RegisterUser.java
│   │       ├── ActivateServlet.java     # OTP account verification
│   │       ├── GoogleLogin.java         # OAuth2 token exchange (manual HTTP)
│   │       ├── LogoutServlet.java
│   │       ├── ProfileServlet.java
│   │       ├── RequestPassword.java     # Forgot password → OTP
│   │       ├── ResetPassword.java
│   │       ├── ResetService.java
│   │       └── InvoiceServlet.java
│   │
│   ├── controllers/                     # Secondary package (4 servlets, NOT in web.xml)
│   │   ├── auth/LogoutController.java
│   │   └── patient/
│   │       ├── HomePage.java
│   │       ├── MedicalHistoryControllers.java
│   │       ├── PatientInvoiceServlet.java
│   │       └── ProfileControllers.java
│   │
│   ├── dal/                             # Data Access Layer
│   │   ├── DBContext.java               # Base class — one JDBC connection per instance
│   │   ├── AppointmentDAO.java          # Booking, queue logic, analytics queries
│   │   ├── AuthDAO.java                 # Login + OTP helpers
│   │   ├── DAOUser.java                 # User registration and profile
│   │   ├── UsersDAO.java                # (overlaps with DAOUser — see Notes)
│   │   ├── UserDAO.java                 # (third user DAO — partial)
│   │   ├── PatientDAO.java              # Patient CRUD + demographics queries
│   │   ├── PatientAccountDAO.java
│   │   ├── StaffAccountDAO.java
│   │   ├── MedicalDAO.java
│   │   ├── DoctorTimeSlotDAO.java
│   │   ├── DoctorUnavailabilityDAO.java
│   │   ├── DoctorHandoffDAO.java
│   │   ├── DoctorSpecialtyDAO.java
│   │   ├── ExaminationPackageDAO.java
│   │   ├── SpecialtyDAO.java
│   │   ├── TimeSlotDAO.java
│   │   ├── InvoiceDAO.java
│   │   ├── InvoiceDetailedDAO.java
│   │   ├── InvoiceItemDAO.java
│   │   ├── DAOPayment.java
│   │   ├── DAOToken.java                # Email verification tokens
│   │   └── DAOHistoryLog.java           # Login audit trail
│   │
│   ├── dto/                             # View-shaped DTOs (query join results)
│   │   ├── AppointmentDTO / AppointmentDetailDTO
│   │   ├── PatientQueueDTO / ReceptionAppointmentDTO
│   │   ├── DoctorDTO / DoctorHandoffDTO / DoctorUnavailabilityDTO
│   │   ├── AssignedDoctorDTO / DoctorAssignDTO
│   │   ├── ExaminationPackageDTO / StaffDTO / TimeSlotDTO
│   │
│   ├── model/                           # Domain beans (map to DB tables)
│   │   ├── User / StaffAccount / Patient / PatientAccount / PatientQueue
│   │   ├── Appointment / MedicalRecord
│   │   ├── DoctorTimeSlot / DoctorUnavailability / DoctorHandoff / DoctorSpecialty
│   │   ├── ExaminationPackage / Specialty / TimeSlot
│   │   ├── Invoice / InvoiceItem / InvoiceDetailed / Payment / Token
│   │   ├── HistoryLog / GoogleAccount
│   │
│   ├── Service/
│   │   ├── MailService.java             # Jakarta Mail SMTP — currently commented out
│   │   └── RandomStringService.java     # OTP / token generation
│   │
│   └── util/
│       ├── DAOUtils.java                # disconnectAll(DBContext...) helper
│       ├── MD5Util.java                 # MD5 password hashing
│       ├── Encryption.java              # Salted SHA-1 (alternative — unused for passwords)
│       └── Testing_connection.java      # Standalone DB connection tester
│
└── web/
    ├── WEB-INF/web.xml                  # Servlet declarations + URL mappings
    ├── login.jsp / signup.jsp / forgotPassword.jsp / resetPassword.jsp
    ├── home.jsp / patientDashboard.jsp / error.jsp
    ├── patient/                         # Patient booking + queue JSPs
    ├── doctor/                          # Doctor queue + schedule JSPs
    ├── manager/                         # Manager analytics + admin JSPs
    ├── receptionist/                    # Receptionist check-in JSPs
    ├── component/ + components/ + cpn/  # Duplicate shared header/footer sets
    └── assets/                          # Bootstrap, ApexCharts, FullCalendar, etc.
```
...
```
User ─────────────── StaffAccount  (role: Doctor | Manager | Receptionist)
     └─────────────── PatientAccount ── Patient  (phone VARCHAR PK — supports walk-ins)

Patient ──── Appointment ──── StaffAccount  (doctor_id FK)
                │         ├── TimeSlot
                │         └── ExaminationPackage ── Specialty
                │
                ├── Invoice ── InvoiceItem         (normalised)
                └── invoices                        (denormalised, 3 fixed line items)

StaffAccount ─── DoctorTimeSlot  (staff_id, slot_id, day_of_week, max_appointments)
             ─── DoctorUnavailability  (specific blocked dates)
             ─── DoctorSpecialty ─── Specialty

DoctorHandoff  (from_doctor_id → to_doctor_id for a given appointment_id)

MedicalRecord  (linked to patient_phone — not to a specific appointment)

Token          (email verification and password reset)
