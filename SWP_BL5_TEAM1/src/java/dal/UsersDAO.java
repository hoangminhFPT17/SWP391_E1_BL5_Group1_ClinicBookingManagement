package dal;import java.sql.PreparedStatement;import java.sql.ResultSet;import java.sql.SQLException;import java.util.ArrayList;import java.util.List;import java.util.logging.Level;import java.util.logging.Logger;import model.User;/** * Data Access Object for User operations * @author ADMIN */public class UsersDAO extends DBContext {    private static final Logger LOGGER = Logger.getLogger(UsersDAO.class.getName());        /**     * Get user profile information by userId     * @param userId the user ID to retrieve     * @return User object with profile data or null if not found     */    public User getUserById(int userId) {        String sql = "SELECT u.user_id, u.email, u.password_hash, u.full_name, u.phone, " +                    "u.is_verified, s.role " +                    "FROM user u " +                    "LEFT JOIN staffaccount s ON u.user_id = s.user_id " +                    "WHERE u.user_id = ?";                try (PreparedStatement stmt = connection.prepareStatement(sql)) {            stmt.setInt(1, userId);                        ResultSet rs = stmt.executeQuery();            if (rs.next()) {                User user = new User();                user.setUserId(rs.getInt("user_id"));                user.setEmail(rs.getString("email"));                user.setPasswordHash(rs.getString("password_hash"));                user.setFullName(rs.getString("full_name"));                user.setPhone(rs.getString("phone"));                user.setIsVerified(rs.getBoolean("is_verified"));                                String role = rs.getString("role");                if (role == null) {                    role = "Patient";                }                user.setRole(role);                                return user;            }        } catch (SQLException ex) {            LOGGER.log(Level.SEVERE, "Error retrieving user profile", ex);        }        return null;    }        /**     * Get user by phone number     * @param phone the phone number to search for     * @return User object with data or null if not found     */    public User getUserByPhone(String phone) {        String sql = "SELECT u.user_id, u.email, u.password_hash, u.full_name, u.phone, " +                    "u.is_verified, s.role " +                    "FROM user u " +                    "LEFT JOIN staffaccount s ON u.user_id = s.user_id " +                    "WHERE u.phone = ?";                try (PreparedStatement stmt = connection.prepareStatement(sql)) {            stmt.setString(1, phone);                        ResultSet rs = stmt.executeQuery();            if (rs.next()) {                User user = new User();                user.setUserId(rs.getInt("user_id"));                user.setEmail(rs.getString("email"));                user.setPasswordHash(rs.getString("password_hash"));                user.setFullName(rs.getString("full_name"));                user.setPhone(rs.getString("phone"));                user.setIsVerified(rs.getBoolean("is_verified"));                                String role = rs.getString("role");                if (role == null) {                    role = "Patient";                }                user.setRole(role);                                return user;            }        } catch (SQLException ex) {            LOGGER.log(Level.SEVERE, "Error retrieving user by phone", ex);        }        return null;    }        /**     * Get user by email
