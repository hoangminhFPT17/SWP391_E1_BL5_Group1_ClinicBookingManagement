...
public class DAOUser extends DBContext {
    ...
    public int registerUser(User user) throws SQLException {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS)) {
            ...
            stmt.setString(2, MD5Util.getMD5Hash(user.getPassword()));
            ...
            stmt.setDate(7, user.getOtpExpire());
            stmt.setDate(8, user.getCreateAt());
...
//     Phương thức lấy thông tin người dùng bằng Email
    ...
    public boolean updateUser(User user) {
        ...
        String sql = "UPDATE `swp_bl5_testing`.`user`\n"
                + "SET\n"
                + "`email` = ?,\n"
                + "`password_hash` = ?,\n"
                + "`phone` = ?,\n"
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            ps.setString(2, MD5Util.getMD5Hash(user.getPassword()));
            ps.setString(3, user.getPhone());
            ps.setString(4, user.getFullName());
            ps.setBoolean(5, user.isIsVertified());
            ps.setString(6, user.getOtpCode());
            ps.setDate(7, user.getOtpExpire());
...
//        }
        int n = 0;
        try {
            n = dao.registerUser(new User(1, "example@example.com", "John Doe", "123456789", "123456789", true, "125",
                    java.sql.Date.valueOf("1990-01-01"), java.sql.Date.valueOf("1990-01-01")));

        } catch (SQLException e) {
            e.printStackTrace();
        }
        dao.Login("example@example.com", "123456789");
        if (n > 0) {
